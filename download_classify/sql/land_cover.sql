...
CREATE MATERIALIZED VIEW ways_lc AS 
    ...
    JOIN lc_sa l ON ST_Intersects(w.geometry, l.geometry) GROUP BY osm_id;
...
CREATE VIEW unknown_lu AS (SELECT osm_id, landscape, geometry FROM ways_rh WHERE landscape IS NULL);
CREATE VIEW known_lu AS (SELECT osm_id, landscape, geometry FROM ways_rh WHERE landscape IS NOT NULL);
UPDATE unknown_lu ul SET landscape = kl.landscape FROM known_lu kl 
	WHERE ST_Touches(ul.geometry, kl.geometry);

DROP VIEW unknown_lu;
DROP VIEW known_lu;

-- Dissolve adjacent land cover polygons with same type of land use
CREATE TABLE land_cover_simple AS SELECT (ST_DUMP(ST_UNION(geometry))).geom, label_modified FROM lc_sa GROUP BY label_modified;
...
CREATE MATERIALIZED VIEW close_to AS
	...
	WHERE ST_DWithin(w.geometry, l.geom, 20) AND NOT ST_Intersects(w.geometry, l.geom)
	GROUP BY osm_id;
...
UPDATE ways_rh w SET landscape = 'coast'
	FROM coastline c WHERE ST_DWithin(w.geometry, c.geometry, 50) 
...
UPDATE ways_rh w SET landscape = 'coast'
	...
	AND ST_DWithin(w.geometry, l.geometry, 50) AND landscape IS NULL;
